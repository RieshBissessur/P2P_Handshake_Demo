...
use crate::eth::secret::Aes128Ctr64BE;
...
impl Ecies {
    ...
    pub fn decrypt<'a>(
        ...
        data_in: &'a mut [u8],
        read_bytes: &mut u16,
    ) -> Result<&'a mut [u8], io::Error> {
        if data_in.len() < 2 {
            return Err(std::io::Error::new(ErrorKind::Other, "Invalid public key")) ;
        }
        ...
        *read_bytes = payload_size + 2;
        ...
        let remote_emphmeral_pub_key = match PublicKey::from_slice(pub_data){
                Ok(remote_emphmeral_pub_key) => remote_emphmeral_pub_key,
        ...
        let remote_tag = Self::calculate_remote_tag(mac_key.as_ref(), iv, encrypted_data, payload_size);
    ...
    pub fn encrypt(&self, data_in: BytesMut, data_out: &mut BytesMut) -> Result<usize, io::Error> {
        ...
        let total_size = match u16::try_from(65 + 16 + data_in.len() + 32) {
    ...
    fn calculate_remote_tag(
        ...
        return H256::from_slice(&hmac.finalize().into_bytes())
    }

    fn calculate_tag(&self, mac_key: &H256, iv: &H128, total_size: &[u8; 2], encrypted_data: &BytesMut) -> Result<H256, io::Error> {
        let mut hmac = match Hmac::<Sha256>::new_from_slice(mac_key.as_ref()){
            Ok(hmac) => hmac,
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid public key")) ,
        };
        hmac.update(iv.as_bytes());
        ...
        return Ok(H256::from_slice(&hmac.finalize().into_bytes()))
    }

    fn calculate_shared_key(&self, public_key: &PublicKey,private_key: &SecretKey) -> Result<H256, io::Error> {
    ...
    fn derive_keys(&self, shared_key: &H256) -> Result<(H128, H256), io::Error> {
        let mut key = [0_u8; 32];
        match concat_kdf::derive_key_into::<sha2::Sha256>(shared_key.as_bytes(), &[], &mut key){
            Ok(_) => {},
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid public key"))  ,
        };
        ...
        return Ok((encryption_key, mac_key));
    }
    ...
    fn prepare_output_data(&self, data_out: &mut BytesMut, ephemeral_key: &SecretKey, iv: &H128, encrypted_data: &[u8], tag: &H256, total_size: u16) -> Result<(), io::Error> {
