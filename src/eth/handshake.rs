...
use sha2::Digest;
use sha3::Keccak256;
use super::{ecies::Ecies, eth_functions::HelloMessage, hash_mac::HashMac, secret::{Aes256Ctr64BE, Secrets}};

const PROTOCOL_VERSION: usize = 5;
const ZERO_HEADER: &[u8; 3] = &[0xC2, 0x80, 0x80];
...
impl Handshake {
    ...
    pub fn encrypt(&self, data_in: BytesMut, data_out: &mut BytesMut) -> Result<usize, io::Error> {
        return self.ecies.encrypt(data_in, data_out);
    }

    pub fn decrypt<'a>(&mut self, data_in: &'a mut [u8], read_bytes: &mut u16) -> Result<&'a mut [u8], io::Error> {
        return self.ecies.decrypt(data_in, read_bytes);
    }

    pub fn derive_secrets(&mut self, ack_body: &[u8]) -> Result<(),io::Error> {
        let rlp = Rlp::new(ack_body);
        let recipient_ephemeral_pubk_raw: Vec<_> = match rlp.val_at(0){
            Ok(recipient_ephemeral_pubk_raw) => recipient_ephemeral_pubk_raw,
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid version")),
        };

        let mut buf = [4_u8; 65];
        buf[1..].copy_from_slice(&recipient_ephemeral_pubk_raw);
        let recipient_ephemeral_pubk = match PublicKey::from_slice(&buf){
            Ok(recipient_ephemeral_pubk) => recipient_ephemeral_pubk,
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid version")),
        };

        // recipient nonce
        let recipient_nonce_raw: Vec<_> = match rlp.val_at(1){
            Ok(recipient_nonce_raw) => recipient_nonce_raw,
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid version")),
        };
        let recipient_nonce = H256::from_slice(&recipient_nonce_raw);

        // ack-vsn
        let ack_vsn: usize= match rlp.val_at(2){
            Ok(ack_vsn) => ack_vsn,
            Err(_) => return Err(std::io::Error::new(ErrorKind::Other, "Invalid version")),
        };
        ...
        // ephemeral-key
        let ephemeral_key = H256::from_slice(&secp256k1::ecdh::shared_secret_point(&recipient_ephemeral_pubk,
                &self.ecies.private_ephemeral_key,
            )[..32],
        );
        ...
        let auth = match self.ecies.auth.as_ref(){
            Some(auth) => auth,
            None => return Err(std::io::Error::new(ErrorKind::Other, "Invalid ecies auth")),
        };
        ...
        let updated_auth = match self.ecies.auth.as_ref(){
            Some(auth) => auth,
            None => return Err(std::io::Error::new(ErrorKind::Other, "Invalid ecies auth")),
        };

        ingress_mac.update(updated_auth);
    ...
    fn write_frame(&mut self, data: &[u8]) -> BytesMut {
        ...
        let secrets = match self.secrets.as_mut(){
            Some(secrets) => secrets,
            None => return BytesMut::default(),
        };
    ...
    pub fn read_frame(&mut self, buf: &mut [u8]) -> Result<Vec<u8>, io::Error> {
        ...
        let secrets = match self.secrets.as_mut(){
            Some(secrets) => secrets,
            None => return Err(std::io::Error::new(ErrorKind::Other, "Invalid secrets")),
        };

        secrets.ingress_mac.compute_header(header);
        if mac != secrets.ingress_mac.digest() {
            return Err(std::io::Error::new(ErrorKind::Other, "Invalid mac"));
        }
        ...
        if frame_mac == secrets.ingress_mac.digest() {
            println!("Hanshake completed succesfully\n Received MAC is valid!!!\n");
        } else {
            return Err(std::io::Error::new(ErrorKind::Other, "Invalid Mac"))
        }
